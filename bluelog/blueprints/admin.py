...
def new_post():
	form = PostForm()
	if form.validate_on_submit():
		post = Post(title=form.title.data, body=form.title.data, category_id=form.category.data
)
		db.session.add(post)
		db.session.commit()
...
def edit_category(category_id):
	...
	if category.id ==1:
		flash('Yous can not edit the default category.', 'warning')
		return redirect(url_for('blog.index'))
	if form.validate_on_submit():
