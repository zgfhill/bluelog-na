...
def show_post(post_id):
	...
	if form.validate_on_submit():
		...
		site = form.site.data
		body = form.body.data
		comment = Comment(author=author, email=email, site=site, body=body, from_admin=from_admin, reviewed=reviewed, post=post)
		...
		db.session.commit()
		if current_user.is_authenticated:
			flash('Comment Tijiaoed.', 'success')
		else:
			flash('你的评论将在审核后显示。')
...
def reply_comment(comment_id):
	...
	if not comment.post.can_comment:
		falsh('Comment is disabled.', 'warning')
